...
// the caller: with a .git suffix bootstrap probes the URL for Dolt data before
// cloning, without it the URL is trusted as a Dolt remote as written.
...
func runBDIn(t *testing.T, bd, dir string, args ...string) (string, string, error) {
	t.Helper()
	return runBDInEnv(t, bd, dir, nil, args...)
}

// runBDInEnv is runBDIn with extra environment entries appended.
func runBDInEnv(t *testing.T, bd, dir string, extraEnv []string, args ...string) (string, string, error) {
	t.Helper()
...
// A .git-suffixed git+file:// sync.remote is probed for Dolt data on the ref
// (#5743 made bootstrap probe such URLs instead of rejecting them). The bare
// repository here holds no Dolt data on any ref, so the plan is a fresh
// database wired to origin; with --ref, origin is wired on that ref and the
// key is persisted, so the first push and the team's other clones agree on
// where the data lives.
func TestEmbeddedGitDataRefBootstrapWiresFreshDatabaseOnRef(t *testing.T) {
	skipUnlessEmbeddedDolt(t)
	bd := buildEmbeddedBD(t)
	...
	ws := bootstrapWorkspace(t, "sync.remote: "+remoteURL+"\n")
	plan := bootstrapPlanOf(t, bd, ws)
	if plan.Action != "init" || plan.SyncRemote != remoteURL || plan.SyncRemoteRef != "" {
		t.Fatalf("without a ref: plan = %+v, want init wired to the remote on the default ref", plan)
	}
	plan = bootstrapPlanOf(t, bd, ws, "--ref", "refs/dolt/units/team-12542")
	if plan.Action != "init" || plan.SyncRemote != remoteURL || plan.SyncRemoteRef != "refs/dolt/units/team-12542" {
		t.Fatalf("with --ref: plan = %+v, want init carrying the remote and the ref", plan)
	}
	if _, stderr, err := runBDIn(t, bd, ws, "bootstrap", "--yes", "--ref", "refs/dolt/units/team-12542"); err != nil {
		t.Fatalf("bd bootstrap --ref on a remote without data: %v\n%s", err, stderr)
	}
	if refs := remoteRefsOf(t, bd, ws); refs["origin"] != "refs/dolt/units/team-12542" {
		t.Fatalf("origin after bootstrap = %q, want the ref the fresh database was wired on", refs["origin"])
	}
	if got := resolveSyncRemoteRefFromDir(filepath.Join(ws, ".beads")); got != "refs/dolt/units/team-12542" {
		t.Fatalf("%s after bootstrap = %q, want the ref persisted beside sync.remote", syncRemoteRefKey, got)
	}

	ws = bootstrapWorkspace(t, "sync.remote: "+remoteURL+"\n"+syncRemoteRefKey+": refs/heads/issue-data\n")
	plan = bootstrapPlanOf(t, bd, ws)
	if plan.Action != "init" || plan.SyncRemoteRef != "refs/heads/issue-data" {
		t.Fatalf("with the key: plan = %+v, want init carrying refs/heads/issue-data", plan)
	}

	// The remote can come from the environment with no config.yaml on disk;
	// the local action then has to create the file for the key to live in,
	// or origin sits on a ref no other clone can learn of.
	noYAML := t.TempDir()
	initGitRepoAt(t, noYAML)
	if err := os.MkdirAll(filepath.Join(noYAML, ".beads"), 0o750); err != nil {
		t.Fatal(err)
	}
	// metadata.json alone makes the directory a workspace bd finds.
	if err := configfile.DefaultConfig().Save(filepath.Join(noYAML, ".beads")); err != nil {
		t.Fatal(err)
	}
	env := []string{"BD_SYNC_REMOTE=" + remoteURL}
	if _, stderr, err := runBDInEnv(t, bd, noYAML, env, "bootstrap", "--yes", "--ref", "refs/heads/issue-data"); err != nil {
		t.Fatalf("bd bootstrap --ref with sync.remote from the environment: %v\n%s", err, stderr)
	}
	if _, err := os.Stat(filepath.Join(noYAML, ".beads", "config.yaml")); err != nil {
		t.Fatalf("config.yaml must be created so the key can be persisted: %v", err)
	}
	if got := resolveSyncRemoteRefFromDir(filepath.Join(noYAML, ".beads")); got != "refs/heads/issue-data" {
		t.Fatalf("%s after bootstrap without a prior config.yaml = %q, want the ref", syncRemoteRefKey, got)
	}
	// The remote goes into the file beside the ref: a committed ref without
	// its remote would send the next clone to its git origin instead.
	if got, _ := config.WorkspaceYamlValue(filepath.Join(noYAML, ".beads"), "sync.remote"); got != remoteURL {
		t.Fatalf("sync.remote after bootstrap from the environment = %q, want %q persisted beside the ref", got, remoteURL)
	}
	if refs := remoteRefsOf(t, bd, noYAML); refs["origin"] != "refs/heads/issue-data" {
		t.Fatalf("origin after bootstrap = %q, want refs/heads/issue-data", refs["origin"])
	}
