...
func isTransientTestHarnessStoreError(err error) bool {
	if err == nil {
		return false
	}
...
func newTestStoreWithRetry(t *testing.T, cfg *dolt.Config) *dolt.DoltStore {
	t.Helper()
	...
	for {
		...
		if testDoltServerPort != 0 {
			if err := testutil.WaitForSQLServer(testDoltServerPort, 5*time.Second); err != nil {
				lastErr = err
			}
		}
		...
		if !isTransientTestHarnessStoreError(err) || time.Now().After(deadline) {
			t.Fatalf("Failed to create dolt store: %v", lastErr)
		}
		if crashErr := testutil.DoltContainerCrashError(); crashErr != nil {
			t.Fatalf("Failed to create dolt store: %v (shared Dolt test server crashed: %v)", lastErr, crashErr)
		}
...
func newTestStoreSharedBranch(t *testing.T, dbPath string, prefix string) *dolt.DoltStore {
	...
	s := newTestStoreWithRetry(t, &dolt.Config{
		Path:         dbPath,
		ServerHost:   "127.0.0.1",
		ServerPort:   testDoltServerPort,
...
func dropTestDatabase(dbName string, port int) {
	...
	//
	// Rapid DROP DATABASE operations on the shared Dolt test container can crash
	// the server mid-suite, producing harness-only flakes that masquerade as
	// product failures. These helper databases are uniquely named and the entire
	// container is torn down at the end of the package run, so leaking them for
	// the lifetime of the test process is acceptable and much more reliable.
	_ = dbName
	_ = port
}
