...
func init() {
	...
	dbProxyChildCmd.Flags().DurationVar(&dbProxyChildIdleTimeout, "idle-timeout", 0, "idle timeout before shutdown; the parent proxy resolves an omitted value to 30s before ever invoking this flag (see bd init --proxied-server-idle-timeout), 0 or negative = never shut down")
