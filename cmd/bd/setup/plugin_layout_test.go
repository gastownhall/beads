...
func TestPluginLayoutUsesSharedBeadsRoot(t *testing.T) {
	...
	readJSONFile(t, filepath.Join(root, ".agents", "plugins", "marketplace.json"), &codexMarketplace)
	if len(codexMarketplace.Plugins) != 1 {
		t.Fatalf("expected one Codex marketplace plugin, got %d", len(codexMarketplace.Plugins))
	}
	if got := codexMarketplace.Plugins[0].Source.Path; got != "./plugins/beads" {
		t.Fatalf("Codex marketplace source = %q, want ./plugins/beads", got)
	}
	...
	requireRepoFile(t, root, "internal", "templates", "skills", "beads", "SKILL.md")
	requireRepoFile(t, root, "internal", "templates", "skills", "beads", "agents", "openai.yaml")
}
