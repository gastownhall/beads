...
func TestCodexHookUserPromptSubmitRecordsSecretFreeReceipt(t *testing.T) {
	...
	var out bytes.Buffer
	input := `{"hook_event_name":"UserPromptSubmit","session_id":"s-codex","prompt":"do not store SECRET-TOKEN"}`
	if err := runCodexHook(context.Background(), codexHookUserPromptSubmit, strings.NewReader(input), &out); err != nil {
		t.Fatalf("UserPromptSubmit: %v", err)
	}
	if len(added) != 1 {
		t.Fatalf("added %d receipts, want 1", len(added))
	}
	if !strings.Contains(added[0], "provider: codex") {
