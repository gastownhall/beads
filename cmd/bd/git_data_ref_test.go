...
// Proxied init routes a URL dolt would not read as git-backed to its git+
// form, as direct init does, and leaves alone what dolt already reads as
// git-backed: the scp form in particular, whose relative path bd's rewrite
// to git+ssh:// would turn absolute. Non-git remotes are untouched.
func TestProxiedInitDoltRemoteURL(t *testing.T) {
	for in, c := range map[string]struct {
		want     string
		refRides bool
	}{
		"https://gitlab.com/org/repo":        {"git+https://gitlab.com/org/repo", true},
		"https://github.com/org/repo.git":    {"https://github.com/org/repo.git", true},
		"git@gitlab.com:org/repo.git":        {"git@gitlab.com:org/repo.git", true},
		"git+ssh://git@host/org/ledgers.git": {"git+ssh://git@host/org/ledgers.git", true},
		"file:///srv/doltremote":             {"file:///srv/doltremote", false},
		"http://myserver:7007/mydb":          {"http://myserver:7007/mydb", false},
		"dolthub://org/db":                   {"dolthub://org/db", false},
	} {
		got := proxiedInitDoltRemoteURL(in)
		if got != c.want {
			t.Errorf("proxiedInitDoltRemoteURL(%q) = %q, want %q", in, got, c.want)
		}
		if ref, ok := refForAdoptedRemote(got, "refs/heads/beads-data"); ok != c.refRides || (ok && ref != "refs/heads/beads-data") {
			t.Errorf("ref on %q (stored as %q) = %q, %v; want rides=%v", in, got, ref, ok, c.refRides)
		}
	}
}
...
func TestSyncRemoteRefIgnoredHint(t *testing.T) {
	...
	// bootstrap and init store a forge URL in its git+ form, which is
	// git-backed whatever its suffix, so the hint judges that form too.
	if hint, ok := syncRemoteRefIgnoredHint("https://gitlab.com/org/repo", "refs/heads/beads-data"); ok {
		t.Errorf("forge URL without .git with a key should not warn, bootstrap probes its git+ form on the ref: %+v", hint)
	}
