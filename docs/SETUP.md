...
Hook-enabled agents (Claude, Gemini) use the `minimal` profile because `bd prime` injects full context at session start. AGENTS-first agents use the `full` profile because their instruction file remains the primary integration surface. Skill-aware agents use `.agents/skills/beads/SKILL.md`, with `.agents/AGENTS.md` telling Codex when to use the skill.
...
| `codex` | `.agents/skills/beads/SKILL.md` + `.agents/AGENTS.md` | Beads agent skill + generated skill guidance |
...
bd setup codex      # Beads agent skill + .agents/AGENTS.md guidance
bd setup mux        # Mux
...
Codex reads repo-local skills from `.agents/skills/`. The Codex setup path installs the generic `beads` agent skill and writes a managed `.agents/AGENTS.md` section that tells Codex to use that skill for Beads workflow guidance.

### Installation

```bash
bd setup codex          # Project Beads skill + .agents/AGENTS.md guidance
bd setup codex --global # Global Beads skill + ~/.agents/AGENTS.md guidance
```
...
- Creates or updates `.agents/AGENTS.md` with a marked section generated by `bd setup codex`

**Global install** (`bd setup codex --global`):
- Creates or updates `~/.agents/skills/beads/SKILL.md`
- Creates or updates `~/.agents/skills/beads/agents/openai.yaml`
- Creates or updates `~/.agents/AGENTS.md` with a marked section generated by `bd setup codex --global`

### Flags

| Flag | Description |
|------|-------------|
| `--check` | Check the Beads agent skill and managed `.agents/AGENTS.md` guidance |
| `--remove` | Remove the Beads agent skill and managed `.agents/AGENTS.md` guidance |
| `--global` | Install/check/remove the global skill and `~/.agents/AGENTS.md` guidance |
...
| Context injection | AGENTS.md | Skill + `.agents/AGENTS.md` | AGENTS.md | Hooks + CLAUDE.md | Hooks + GEMINI.md | Rules file | Config file |
...
| Standard format | Yes (AGENTS.md) | Yes (`.agents/AGENTS.md`) | Yes (AGENTS.md) | No (proprietary) | No (proprietary) | No (proprietary) | No (proprietary) |
