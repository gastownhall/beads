...
```bash
bd init --skip-agents    # Skip AGENTS.md/CLAUDE.md, keep agent skill + hooks
bd init --stealth        # Invisible mode: skips tracked AGENTS/hooks, keeps local Beads agent skill excluded
```
