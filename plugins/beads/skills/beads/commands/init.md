...
Use `bd init --prefix <prefix>` when a prefix is provided, or `bd init` otherwise, to set up a new beads database.
...
If beads is already initialized, inform the user and show project stats with `bd stats --json`.
