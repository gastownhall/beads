...
- Check workspace health with `bd doctor`
- Check database location with `bd where`
- Check CLI version with `bd version`
- Use `--json` for agent-readable output when diagnosing command failures
...
**Symptom:**
Dependencies end up reversed from intended.
...
**Root Cause:**
`bd dep add A B` means "A depends on B".
...
If the **beads skill** provides incorrect guidance:

1. **Check skill version:**
   ```bash
   head -20 ~/.claude/skills/beads/SKILL.md 2>/dev/null || true
   head -20 ~/.codex/skills/beads/SKILL.md 2>/dev/null || true
   ```

2. **Report via the user's normal feedback channel** or GitHub
...
| Dependencies backwards | Remember `bd dep add A B` means A depends on B |
