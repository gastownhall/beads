...
**Example (CLI + atomic claim):**

Working on "Implement checkout flow" (checkout-1), discover payment validation security hole:

1. Create bug issue: `bd create "Fix: payment validation bypasses card expiry check" -t bug -p 0 --json`
2. Link discovery: `bd dep add payment-bug-2 checkout-1 --type discovered-from --json`
3. Block current work: `bd update checkout-1 --status blocked --notes "Blocked by payment-bug-2: security hole in validation" --json`
4. Start new work atomically: `bd update payment-bug-2 --claim --json`
...
**Pattern (CLI)**:
1. **Check what's ready**: Use `bd ready --json` to see available work
2. **Check what's stuck**: Use `bd blocked --json` to understand blockers
3. **Check recent progress**: Use `bd list --status closed --json` to see completions
4. **Read detailed context**: Use `bd show <id> --json` for the issue you'll work on
5. **Claim issue**: Use `bd update <id> --claim --json` for atomic start
