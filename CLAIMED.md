...
| Versioned beads Phase 2 — migration slot 0068 (`0068_add_attribution_status`: `issue_versions.attribution_status`, design §16.4 / #6135); #6008 also currently carries 0068 — see the label-registry row, whichever of #6008 / #6358 merges second renumbers | @quad341 | 2026-09-06 | branch `builder/be-0uifx` on quad341/beads-sec003-contrib (head `c249019da`); PR #6358 (branch `deploy/be-764ey-gate`), review and release gate passed, stacked on #6147 |
| Versioned beads Phase 3 — migration slots **0069–0070** (`0069_add_epoch_minted_addresses`: R20 epochs; `0070_add_removed_restriction`: R7.1 as-of read; R16 expected-revision CAS needs no slot, it lands on `issues.current_revision` from 0067; design #5898 / #6136) | @quad341 | 2026-09-10 | branches on quad341/beads-sec003-contrib; PR to follow after #6358 |
| BDP bead graph — migration slots **0071 and later** (five replicated-table files: scope, types, beads, links, ledger; plus the dolt-ignored `graph_authority_lease` main-series file and its `ignored/` twin); design #6154 | @donnabox | 2026-09-07 | `feat/bead-graph` on donnabox/beads; docs PR #6154 (all P-1 rulings in); P0 (contracts + pinned wire) as a separate PR next; the migrations land in P1 behind a replication/merge ADR. Start moved 0069→0071 on 2026-09-16 by @quad341, applying [@donnabox's own next-free-numbers rule](https://github.com/gastownhall/beads/pull/6149#issuecomment-5585888296) after Phase 3 took 0069–0070; @donnabox, correct this row here if that reads wrong |
| Opt-in label vocabulary registry — migration slot: next contiguous number at merge time (the branch currently carries `0068_create_label_definitions`, the number Phase 2's row holds); #6010 inherits whatever number #6008 lands on | @marcodelpin | 2026-09-18 | branch `pr/label-definitions-storage` on marcodelpin/beads; PR #6008 (approved), #6010 stacked on it and inheriting the number. Proposal: whichever of #6008 / #6358 merges second rebases and renumbers to the next contiguous number before landing, because `TestAllMigrationsSQLAppliesThroughDoltCLIAndRecordsLatestVersion` requires `COUNT(*) == LatestVersion()` (measured on #6008). Sequencing between #6008 and #6358 is @julianknutsen's call. Transcribed by @quad341 from [@marcodelpin's proposed row](https://github.com/gastownhall/beads/pull/6149#issuecomment-5724342038); @marcodelpin, correct it here if it reads wrong |
