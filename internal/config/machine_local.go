...
func setMachineLocalYamlConfig(configPath, key, value string) error {
	...
	return setYamlConfigAtPath(localPath, key, value)
}
...
func unsetMachineLocalYamlConfig(configPath, key string) error {
	...
	updated, err := commentOutYamlKeyAnyForm(string(content), key)
	if err != nil {
		return err
	}
...
func migrateMachineLocalKeys(configPath, localPath string) error {
	...
	for _, key := range keys {
		...
		tracked, err = commentOutYamlKeyAnyForm(tracked, key)
		if err != nil {
			return err
		}
	}
...
// SetMachineLocalYamlConfigInDir writes key to the machine-local sidecar beside
// beadsDir's config.yaml, and lifts any existing tracked value for it into the
// sidecar on the way.
//
// This is the EXPLICIT half of the machine-local split (bd-zj95 / #6125). The
...
// UnsetMachineLocalYamlConfigInDir comments key out of the sidecar beside
// beadsDir's config.yaml. The tracked config.yaml is left alone: a value there
// is a shared default that only an explicit edit should remove.
func UnsetMachineLocalYamlConfigInDir(beadsDir, key string) error {
	return unsetMachineLocalYamlConfig(filepath.Join(beadsDir, "config.yaml"), key)
}

// SetMachineLocalYamlConfig is SetMachineLocalYamlConfigInDir for the project
// config.yaml the caller is standing in — the shape `bd config set` needs.
...
// UnsetMachineLocalYamlConfig is UnsetMachineLocalYamlConfigInDir for the
// project config.yaml the caller is standing in.
func UnsetMachineLocalYamlConfig(key string) error {
	configPath, err := findProjectConfigYaml()
	if err != nil {
		return err
	}
