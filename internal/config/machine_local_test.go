...
func TestMachineLocalKeysNeverReachTrackedConfig(t *testing.T) {
	// Both ROUTED writers are covered. They resolve the target workspace
	// differently — SetMachineLocalYamlConfig discovers it,
	// SetMachineLocalYamlConfigInDir is handed it — and `bd config set`, the
	// command that produced the reported defect, goes through the discovering
	// one.
	//
	// The plain writers (SetYamlConfig, SetYamlConfigInDir) are deliberately
	// NOT covered here: since this change they write exactly where they are
	// told, so #6574's round-trip guarantees hold unchanged and a caller that
	// names a file gets that file. The routing decision sits with the caller
	// that knows the user's intent; TestNoBdWriterSendsAMachineLocalKeyToTheTrackedAPI
	// pins that no bd code path skips it.
...
func TestSharedKeysStillReachTrackedConfig(t *testing.T) {
	shared := []struct{ key, value string }{
		{"dolt.auto-start", "true"},      // fleet policy, committed on purpose (differs from the fixture)
	...
	for _, tc := range shared {
		t.Run(tc.key, func(t *testing.T) {
			...
			// Assert the PROPERTY (the tracked file carries the value), not the
			// proxy (bytes changed): since #6574 the writer updates an existing
			// spelling in place, so writing a value the file already has is a
			// byte-level no-op and the proxy failed on a correct write.
			_ = before
			if got := GetStringFromDir(beadsDir, tc.key); got != tc.value {
				t.Errorf("after writing SHARED key %q, GetStringFromDir = %q, want %q", tc.key, got, tc.value)
			}
...
`
	...
	if err := SetMachineLocalYamlConfigInDir(beadsDir, "dolt.port", "3306"); err != nil {
...
func TestMigrationRunsOnlyOnce(t *testing.T) {
	...
	if err := SetMachineLocalYamlConfigInDir(beadsDir, "dolt.user", "bd"); err != nil {
...
func TestUnsetMachineLocalKeyLeavesTrackedConfigAlone(t *testing.T) {
	...
	if err := SetMachineLocalYamlConfigInDir(beadsDir, "dolt.mode", "server"); err != nil {
		t.Fatalf("set: %v", err)
	}
	before := readFile(t, configPath)

	t.Chdir(filepath.Dir(beadsDir))
	if err := UnsetMachineLocalYamlConfig("dolt.mode"); err != nil {
		t.Fatalf("UnsetMachineLocalYamlConfig: %v", err)
	}
...
func TestCommentOutYamlKeyAnyForm(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("commentOutYamlKeyAnyForm(%q): %v", tc.key, err)
			}
			if got != tc.want {
...
`
	...
	for i := 0; i < runs; i++ {
		beadsDir, configPath, localPath := newWorkspace(t, tracked)
		if err := SetMachineLocalYamlConfigInDir(beadsDir, "dolt.debug", "true"); err != nil {
			t.Fatalf("run %d: SetMachineLocalYamlConfigInDir: %v", i, err)
		}
