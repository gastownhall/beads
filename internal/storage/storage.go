...
// manually-set status='blocked' (internal/types/types.go) after every 'blocks'
// dependency closed, or with none ever recorded. Unlike BlockedRecomputer,
// which repairs the derived is_blocked column, nothing else in bd ever clears
// this status on its own (be-ntbxt) — callers should type-assert to this
// interface for the 'bd recompute-blocked --status' check.
