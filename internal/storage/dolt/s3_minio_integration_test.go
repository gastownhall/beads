...
func captureS3OperationStderr(t *testing.T, operation func() error) (string, error) {
	...
	// Restore on every exit path, including a panic or t.Fatal inside
	// operation(); otherwise os.Stderr stays pointed at the capture pipe and
	// everything the test binary writes from then on, the panic trace
	// included, is swallowed. Closing the writer is what lets the reader
	// goroutine finish, so it happens here too, before the normal path reads
	// the channel.
