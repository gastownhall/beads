...
// proves the ownership-handoff crash window before the child has written PID
// or port state. The retry may recover exactly that nonce-bound child and
// must not spawn or adopt another listener.
...
// TestLifecycle_StrictRecoverOnlyFindsStoppedPrelistenChild proves that
// rollback can recover an exact nonce child before it has bound the endpoint
// or written lifecycle state, then retire it through a process-birth handle.
