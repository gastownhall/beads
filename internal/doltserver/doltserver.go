...
func StartWithOptions(beadsDir string, options StartOptions) (*State, error) {
	...
	// Strict only: ordinary Start must keep falling through to KillStaleServers
	// and reclaimPort, which adopt a live server whose PID file was lost.
	if options.RequireFresh && cfg.Port > 0 && !isPortAvailable(cfg.Host, cfg.Port) {
