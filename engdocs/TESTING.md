...
| Hook shims against real timeout implementations | `nix build .#checks.x86_64-linux.hook-timeout-backends -L` | After changing the hook generator in `cmd/bd/hooks.go` (then `make githooks-regen`) or anything under `.githooks/`. Runs the tracked managed sections against GNU coreutils, uutils, busybox and toybox `timeout`, with and without Perl, under dash, bash and busybox ash. About one deadline of wall time; needs no Go build. |
