...
Revision v16: 2026-09-10 formal-review correction carries the already-ruled A10
...
one). `bd --graph-mode link serve` is the strict command over that server: it mints and
...
ledger head and, on the shared-database leg, a lease row the mutation itself
must update. The store refuses a clone without local authority, a restore whose
head is absent from the witness, and a promotion elsewhere; no caller can
supply the witness. Copies carrying coherent local state are an operator-managed
hazard, including a solo store copy; the store cannot infer their provenance.
A shared database is fenced by that
lease. Under A9, a configured remote neither grants nor removes authority;
hazard R's publication mechanism is deferred to the write-profile ADR. A
shared database that minted locally remains authoritative with a remote.
The A10 exception is an embedded workspace with no remote and no server: it has local authority and
local link-mode verbs, but serves no HTTP. Its workspace gate satisfies the
solo lease; appearance of a shared database or remote makes it a client host
under the existing rotation and steal rules. A10 also preserves the replication
case: a solo store later pushed to a remote counts as minted locally and remains
authoritative until a foreign delta appears. The full A10 row governs both cases.

## 2. Corrections to the plan and ruled amendments (read this first)
...
internal/storage/authority/      THE WITNESS MANAGER (clone-local half; no SQL)
  ...
  Load: plain read; a Pending record → recovery by evidence (the ledger carries the op id?
    ...
    any assertion; run by admin verbs and bd --graph-mode link serve only; bd serve and CLI reads treat
...
- **`graphops.Admin`** — `Promote` (precondition: Scope row CAS + the
  ...
  and `bd --graph-mode link restore` under the **exclusive** gate; `httpapi.GraphConfig`
  has no field for it.
...
```text
client ──GET /acme/beads/x──▶ internal/httpapi (bd serve | bd --graph-mode link serve; UOW leg)
...
| 8 changefeed | P3 |
| 9 authority | store-owned witness in every transaction (A1); fences by hazard (A7), hazard S under A9, and the solo workspace gate under A10; contract cases incl. clone, restore, copied witness, expired-lease mutation, nonce mismatch, graph delta under an unchanged tuple, revert when HEAD moved, each phase's recovery |
...
| 12 store/Scope/client | `bd init` migrations only; among serving commands only `bd --graph-mode link serve` mints (staged); A10 solo mint trigger is a P1 mechanism; `bd --graph-mode link client` (A6); A8 options; registered backends absent in v0 |
