...
- **Testing**: All new features need tests (`make test-short` for the fast local/PR-like lane, `make test` for the broader local lane, `make test-full-cgo` when validating full CGO paths)
...
1. **Run tests**: `make test-short` for the fast local baseline, then `make test` when you need the broader non-short local lane
