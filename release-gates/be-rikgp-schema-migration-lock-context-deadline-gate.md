...
- **Signal UX:** an interrupt received mid-migration is no longer swallowed silently — it is disclosed on stderr once the pass completes. Deliberately *not* reported live: `MigrateUp` writes its own progress to the same stderr, so a watcher goroutine would be a second concurrent writer to a writer that is not safe for it (the first attempt at this was, and `-race` caught it on the full-package run).
- **Criterion 7 note:** the diff is now 3 files rather than 2 (the gate doc itself included). The feature theme is unchanged.
- **Test evidence at the new tip:** `go build ./...` and `go vet ./internal/storage/schema/` clean, `gofmt -l` empty. `go test ./internal/storage/schema/ -race` run repeatedly on the full package — **14/14 clean**; the run count is deliberate, since the stderr race above surfaced in roughly one run in four and a single green run would not have distinguished the fix from luck. The heal-path regression was confirmed genuinely RED before the fix (heal skipped, `DOLT_RESET` never reached) and GREEN after.
