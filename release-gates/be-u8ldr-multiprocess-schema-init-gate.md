...
| 7 | Single feature theme | PASS | `git show --stat e2a7bbb90b`: 1 file changed (`internal/storage/dolt/initschema_multiprocess_test.go`), 16 insertions(+), 18 deletions(-). One coherent theme: remove dead decoy-listener test code and swap to the retry-safe schema-init call, matching production behavior (`store.go:2659`) and sibling test convention (`initschema_concurrent_test.go`). No scope creep. |
