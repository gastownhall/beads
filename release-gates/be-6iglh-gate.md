...
| 2 | Acceptance criteria met | PASS | Against `be-vbhpf`'s Done-when list: (1) `dolt.Config` carries an explicit `ClassifiedRead` signal set only from `cmd/bd/main.go` — `internal/storage/dolt/store.go` (+9), `cmd/bd/main.go` (+4). (2) `wakeExpiredDefers` no longer skips on classification-only read-only open, doc comment updated — `internal/storage/dolt/queries.go` (+19/-1). (3) Server-mode test proves an expired dated defer returns to open on `bd ready` and `bd list --ready`, failing on old code / passing after — `TestServerModeDeferAutoWake/expired_dated_defer_wakes_on_ready` + `/expired_dated_defer_wakes_on_list_ready`, both PASS (see criterion 3). (4) Strict-readonly read-only still does not sweep — `TestDeferWakeSweepEligibleHonorsClassifiedRead/strict_read-only_never_sweeps` PASS (pure-function case: `deferWakeSweepEligible(readOnly=true, classifiedRead=false)` = false; round-2 adds the equivalent live proof at the command layer, `TestServerModeDeferAutoWake/expired_dated_defer_stays_hidden_under_strict_readonly`). Preview and foreign-project opens are not separately exercised by any diff-owned test; they rest on code structure instead — `ClassifiedRead` is set to `true` only at the single call site `cmd/bd/main.go:1485` (`policy.readOnly && !readonlyMode && !previewMode`), so an explicit preview open (`!previewMode` false) and a foreign-project lookup (which does not go through this call site at all) both leave `ClassifiedRead` at its `false` zero-value, taking the same never-sweep path the tested strict-readonly case does (bee-ghosttrack review findings 2 and 3, PR #6424 round 1). (5) Dateless defer still never woken — `TestServerModeDeferAutoWake/dateless_defer_never_wakes` PASS. (6) Existing read-only-command tests (incl. the auto-prune one at `cmd/bd/events_journal.go:116-121`) still pass — `cmd/bd` package `ok` in both the full-suite baseline and the targeted container run, no regressions. (7) "Manual check against a real server-mode store: an issue deferred with a past date is open after one `bd ready`" — not hand-performed separately; `TestServerModeDeferAutoWake/expired_dated_defer_wakes_on_ready` exercises this exact scenario against a real `dolthub/dolt-sql-server:2.2.0` container in genuine server mode (not a mock), which is the automated equivalent of the manual check and is noted here as such rather than silently substituted. |
| 3 | Tests pass | PASS | `test_cmd: TEST_COVER=1 ./scripts/test.sh` (`go test -p 4 -parallel 4 -timeout 25m -covermode=atomic -coverprofile ... ./...`) — `test_cmd_scope: full-suite`. `test_counts`: every package in the tree reports `ok` or "no test files/no statements" — **0 FAIL**, exit code 0, total coverage 39.1%. `diff_tests_executed`: run separately (`BEADS_TEST_ENV_RUN_DOLT=1 BEADS_TEST_PROXIED_SERVER=1`, rootless podman, `dolthub/dolt-sql-server:2.2.0`), because the default full-suite env self-skips dolt-server tests to match real PR-CI — that env gate applies at the package level (both packages' `TestMain` start a server), not because every diff-owned test itself touches one: `TestDeferWakeSweepEligibleHonorsClassifiedRead` (`internal/storage/dolt/readonly_server_policy_test.go`, new func) is a pure-function table test of `deferWakeSweepEligible` directly — no store, no server, no container — PASS, 4/4 subtests (`writable_open_sweeps`, `strict_read-only_never_sweeps`, `classified-read_still_sweeps_despite_readOnly`, `classifiedRead_without_readOnly_stays_eligible`), package `ok` 5.957s. `TestServerModeDeferAutoWake` (`cmd/bd/defer_wake_server_test.go`) is genuinely container-backed — it drives a real `dolthub/dolt-sql-server:2.2.0` server through `bd init --server` — PASS, 5/5 subtests (`expired_dated_defer_wakes_on_ready`, `expired_dated_defer_wakes_on_list_ready`, `dateless_defer_never_wakes`, `future_dated_defer_stays_hidden`, and round-2's `expired_dated_defer_stays_hidden_under_strict_readonly` — the strict-`--readonly` live proof at the command layer, PR #6424 review round 1, bee-ghosttrack finding 1), package `ok` 12.427s. Both diff-owned test files verified exhaustively (`grep '^func Test'`) — no other diff-owned test function exists in either file. Re-run `TESTCONTAINERS_RYUK_DISABLED=true` (host load average 10-13 during this session was starving the testcontainers-go reaper's own startup, unrelated to this diff; the shared dolt-sql-server container itself always started cleanly). |
...
| `TestDeferWakeSweepEligibleHonorsClassifiedRead` (`internal/storage/dolt`, pure-function table test — package `TestMain` starts a server, but this test calls `deferWakeSweepEligible` directly) | PASS, 4/4 subtests, `ok internal/storage/dolt 5.622s`. |
...
holds at the new HEAD: both diff-owned test files pass in full (see table
above) — `TestServerModeDeferAutoWake` genuinely container-backed,
`TestDeferWakeSweepEligibleHonorsClassifiedRead` a pure-function table test
whose package happens to require the same env gate since its `TestMain`
starts a server.
