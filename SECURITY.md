...
- The `.beads/` directory contains server state files (PID, port) and defaults to owner-and-group permissions (0770 for directories and 0660 for files). Every member of that Unix group is trusted; world access remains unsafe because it lets unrelated local users tamper with process lifecycle.
