...
- **`bd export` `updated_at` timezone corruption** - All blocked-state recompute UPDATEs (`is_blocked` toggles in `internal/storage/issueops/blocked_state.go` and `dependencies.go`) on both the `issues` and `wisps` tables omitted `updated_at`, so the schema's `ON UPDATE CURRENT_TIMESTAMP` auto-filled it from the embedded Dolt driver's local-time query clock — emitting local wall-clock values mislabeled with a `Z` (UTC) suffix and corrupting UTC-based comparisons. NoHistory beads (stored in the `wisps` table with `ephemeral=0`) are included in `bd export` output and were affected. All five paths now set `updated_at = time.Now().UTC()` explicitly. ([#4298](https://github.com/gastownhall/beads/issues/4298))
## [1.0.5] - 2026-05-28
