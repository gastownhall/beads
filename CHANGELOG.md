...
- **`bd recall`, `bd forget`, `bd remember <bare-key>` and
  `GET /v0/beads/memories/{key}` now treat an empty-valued memory as present**
  ...
  band with an empty string was invisible even though its row existed. `bd
  recall` printed "No memory with key" and exited 1, `bd forget` refused to
  delete it, `bd remember <bare-key>` refused to recall it, and the HTTP door
